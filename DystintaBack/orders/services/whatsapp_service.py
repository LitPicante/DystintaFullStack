...
def normalize_whatsapp_phone(value: str | None) -> str:
    """Devuelve solo digitos para que Evolution API reciba un numero limpio."""
...
def evolution_is_configured() -> bool:
    return bool(_setting("EVOLUTION_API_URL") and _setting("EVOLUTION_API_KEY") and _setting("EVOLUTION_INSTANCE_NAME"))
...
def build_order_status_message(order: Order) -> str:
    ...
    return template.format(name=order.name or "cliente", status=order.status, service=order.service)


def send_text_message(phone: str, text: str) -> WhatsAppSendResult:
    """Envia un mensaje de texto usando Evolution API por HTTP interno/local."""
    try:
        import requests
    except ImportError as exc:
        logger.exception("whatsapp.send.requests_missing")
        raise WhatsAppDeliveryError("Python package 'requests' is not installed") from exc

    number = normalize_whatsapp_phone(phone)
    if not number:
        logger.warning("whatsapp.send.skipped_missing_phone")
        return WhatsAppSendResult(sent=False, reason="missing_phone")

    base_url = _setting("EVOLUTION_API_URL", "http://127.0.0.1:8080").rstrip("/")
    api_key = _setting("EVOLUTION_API_KEY")
    instance_name = _setting("EVOLUTION_INSTANCE_NAME", "dystinta-main")
    timeout = int(getattr(settings, "EVOLUTION_API_TIMEOUT", 15) or 15)

    if not api_key:
        logger.warning("whatsapp.send.skipped_missing_api_key")
        return WhatsAppSendResult(sent=False, reason="missing_api_key")
    ...
    try:
        ...
        logger.exception("whatsapp.send.timeout number=%s", number)
        raise WhatsAppDeliveryError("Evolution API timeout") from exc
    except requests.RequestException as exc:
        logger.exception("whatsapp.send.failed number=%s", number)
        raise WhatsAppDeliveryError("Evolution API request failed") from exc
    ...
    logger.info("whatsapp.send.ok number=%s status_code=%s", number, response.status_code)
    return WhatsAppSendResult(sent=True, response=data)
...
def send_order_status_message(order: Order) -> WhatsAppSendResult:
    ...
    if not order.phone:
        logger.warning("whatsapp.order_status.skipped_missing_phone order_id=%s", order.pk)
        return WhatsAppSendResult(sent=False, reason="missing_phone")
    ...
    try:
        ...
        logger.exception("whatsapp.order_status.failed order_id=%s status=%s", order.pk, order.status)
        return WhatsAppSendResult(sent=False, reason="delivery_error")
...
def extract_phone_from_webhook(payload: dict[str, Any]) -> str:
    ...
    return normalize_whatsapp_phone(str(remote_jid).split("@", 1)[0])
...
def append_internal_note(order: Order, note: str) -> None:
    ...
    order.notes = f"{order.notes}\n{entry}".strip() if order.notes else entry
...
def find_latest_order_waiting_for_approval(phone: str) -> Order | None:
    ...
    # El telefono puede venir con espacios, +595 u otros separadores en DB.
    candidates = Order.objects.filter(status=Order.STATUS_APROBACION_CLIENTE).order_by("-status_updated_at", "-updated_at")
    for order in candidates:
        if normalize_whatsapp_phone(order.phone).endswith(digits) or digits.endswith(normalize_whatsapp_phone(order.phone)):
            return order
    return None


def process_evolution_webhook(payload: dict[str, Any]) -> dict[str, Any]:
    """Procesa respuestas 1/2 de clientes y actualiza el pedido si corresponde."""
    logger.info("whatsapp.webhook.received event=%s instance=%s", payload.get("event"), payload.get("instance"))
    ...
    response_code = (re.match(r"^\s*([12])\b", text or "") or [None, None])[1]

    if response_code not in {"1", "2"}:
        logger.info("whatsapp.webhook.ignored_invalid_response phone=%s text=%s", phone, text)
        return {"processed": False, "reason": "invalid_response"}

    order = find_latest_order_waiting_for_approval(phone)
    if not order:
        logger.info("whatsapp.webhook.ignored_no_pending_order phone=%s", phone)
        return {"processed": False, "reason": "no_order_waiting_approval"}

    if order.status != Order.STATUS_APROBACION_CLIENTE:
        logger.info("whatsapp.webhook.ignored_wrong_status order_id=%s status=%s", order.pk, order.status)
        return {"processed": False, "reason": "wrong_status", "order_id": order.pk}

    if response_code == "1":
        order.status = Order.STATUS_PRODUCCION
        append_internal_note(order, "Cliente aprobo el diseno por WhatsApp.")
        action = "approved"
        logger.info("whatsapp.webhook.customer_approved order_id=%s phone=%s", order.pk, phone)
    else:
        order.status = Order.STATUS_DISENO
        append_internal_note(order, f"Cliente rechazo el diseno por WhatsApp. Respuesta: {text}")
        action = "rejected"
        logger.info("whatsapp.webhook.customer_rejected order_id=%s phone=%s", order.pk, phone)

    order.save(update_fields=["status", "notes", "updated_at"])
    return {"processed": True, "action": action, "order_id": order.pk, "new_status": order.status}
